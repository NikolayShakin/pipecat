...
class AsteriskWSServerOutputTransport(BaseOutputTransport):
    ...
    def __init__(self, transport: BaseTransport, params: AsteriskWSServerParams, **kwargs):
        ...
        self._audio_buffer: asyncio.Queue[OutputAudioRawFrame] = asyncio.Queue(maxsize=self._params.local_audio_buffer_frames)
        ...
        self._audio_buffer_bytes_buffered: int = 0  # current size of data buffered in _audio_buffer in bytes
        ...
        self._max_remote_audio_buffer_bytes: int = 0 # calculated based on optimal frame size and _max_remote_audio_buffer_frames
        ...
        self._remote_audio_buffer_resume_threshold_bytes: int = 0 # calculated based on optimal frame size and _remote_audio_buffer_resume_threshold
    ...
    async def set_client_connection(self, websocket: Optional[websockets.WebSocketServerProtocol]):
        ...
        """
        if self._websocket != None and websocket != None:
            logger.warning(f"We already have a client connected, ignoring the new connection from {websocket.remote_address}")
            return
        self._websocket = websocket
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        elif isinstance(frame, InputTransportMessageFrame) and frame.message.get("event", None) == "MEDIA_START":
    ...
    async def _handle_media_start(self, message: dict):
        ...
        logger.debug(f"{self} remote audio buffer max size set to {self._max_remote_audio_buffer_bytes} bytes")
        ...
        logger.debug(f"{self} remote audio buffer resume threshold set to {self._remote_audio_buffer_resume_threshold_bytes} bytes")
        ...
        if self._params.initial_jitter_buffer_ms > 0:
            ...
            logger.debug(f"{self} initial jitter buffer size set to {self._initial_jitter_buffer_bytes} bytes")
        ...
        try:
            ...
            logger.error(f"{self} exception sending START_MEDIA_BUFFERING command: {e.__class__.__name__} ({e})")
        ...
        if payload:
            ...
            try:
                ...
                if self._params.initial_jitter_buffer_ms > 0 and not self._initial_jitter_buffer_is_filled:
                    self._audio_buffer_bytes_buffered = self._audio_buffer_bytes_buffered + len(payload)
            except Exception as e:
    ...
    async def _buffer_consumer(self):
        ...
        if self._params.initial_jitter_buffer_ms > 0:                        
            ...
            while not self._initial_jitter_buffer_is_filled:
                if self._audio_buffer_bytes_buffered < self._initial_jitter_buffer_bytes:
                    ...
                    logger.debug(f"{self} Initial jitter buffer filled, starting to send audio frames")
        ...
        while True:
            ...
            try:
                ...
                logger.error(f"{self} exception sending buffered data: {e.__class__.__name__} ({e})")
    ...
    async def _buffer_state_monitor(self):
        ...
        while True:
            ...
            await asyncio.sleep(self._ptime) # Check every ptime seconds, it's Ok in term of frequency, but
            ...
            if self._remote_audio_buffer_bytes > 0:
                logger.trace(f"{self} remote audio buffer size: {self._remote_audio_buffer_bytes} bytes")
            ...
            if self._audio_buffer_consumer_can_send.is_set():
                ...
                not (self._remote_audio_buffer_is_full or self._audio_buffer.empty()) and 
                self._remote_audio_buffer_bytes < self._remote_audio_buffer_resume_threshold_bytes
                ):
                    logger.trace(f"{self} resuming buffer consumer as remote buffer has enough space and local buffer has data")
                    self._audio_buffer_consumer_can_send.set()
    ...
    async def _flush_remote_audio_buffer(self):
        ...
        try:
            ...
            logger.error(f"{self} exception sending FLUSH_MEDIA command: {e.__class__.__name__} ({e})")
